  ...
  public static class Die {
    ...
    private static List<Die> createDieList(String... allDieSides) {
      ...
      for (String dieSides : allDieSides) {
        dice.add(new Die(dieSides));  // creates an ArrayList of the elements in allDieSides
      }
      ...
      return Collections.unmodifiableList(dice);  // makes the ArrayList unmodifiable
    }
  ...
  public BoardImpl() {
    ...
    for (int row = 0; row < BOARD_ROWS; row++) {
      for (int column = 0; column < BOARD_COLUMNS; column++) {
        ...
        /* removes a sequence of 6 letters from ALL_DICE and assigns it to nextDie */
        _cells[row][column] = nextDie.throwDie();  // chooses a random letter from the 6-letter sequence and assigns it to the cell
      }
    }
  }
